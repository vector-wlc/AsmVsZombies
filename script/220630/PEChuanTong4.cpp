...
void UseClover()
{
    ...
    if (_clover->isUsable()) {
        ...
        for (int index = 0; index < GetMainObject()->zombieTotal(); ++index) {
            // 如果气球僵尸快飞到家了
            if (zombie[index].isExist() && zombie[index].type() == QQ_16 && !zombie[index].isDead() && zombie[index].abscissa() <= 2.5 * 80) {
                // 种植三叶草
...
void UseSquash()
{
    ...
    if (seed[_SQUASH].isUsable()) {
        ...
        for (index = 0; index < zombie_count_max; ++index) {
            ...
            if (zombie[index].isExist() && !zombie[index].isDead() && zombie[index].hp() + zombie[index].oneHp() > 500) {
                zombie_type = zombie[index].type();
                zombie_abscissa = zombie[index].abscissa();
                zombie_row = zombie[index].row() + 1;
                if (zombie_type == 7)
                    zombie_abscissa += 40;
                if ((zombie_type == 23 || zombie_type == 32 || zombie_type == 7) && (zombie_row == 1 || zombie_row == 6)) {
        ...
        if (zombie_min_position.min_row != 0) {
            ...
            if (Asm::getPlantRejectType(WG_17, use_squash_grid.row - 1, use_squash_grid.col - 1) == Asm::NIL) { // 能种倭瓜再种
