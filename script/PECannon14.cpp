...
void PECannon14()
{
    ...
    {
        SetTime(-15, wave);
        pao_operator.tryPao({{2, 9}, {5, 9}});
        SetTime(-15 + 110);
        pao_operator.tryPao({{1, 7.5}, {5, 7.5}});
    ...
    for (auto wave : {3, 6, 9, 12, 15, 18})
    {
        ...
        pao_operator.tryPao({{1, 2.4}, {5, 2.4}});
        SetTime(1290 - 298 + 30);
        ice_filler.coffee();
    }
    ...
    {
        SetTime(150, wave);
        pao_operator.tryPao({{1, 7.2}, {5, 7.2}});
        SetTime(1670 - 373 - 200);
        pao_operator.tryPao({{2, 8.5}, {5, 8.5}});
        SetDianCai();
        SetTime(1670 - 373 - 200 + 220);
        pao_operator.tryPao({{1, 7.3}, {5, 7.3}});
    }
    ...
    for (auto wave : {9, 19})
    {
        ...
        pao_operator.tryPao({{1, 8.5}, {5, 8.5}});
        SetDianCai();
        SetTime(time + 300 + 715 + 220);
        pao_operator.tryPao({{1, 8.5}, {5, 8.5}});
        pao_operator.tryRecoverPao({{2, 9}, {5, 9}});
    }
    ...
    // wave 20
    SetTime(-150, 20);
    pao_operator.tryPao(4, 7);
    SetTime(-95);
    pao_operator.tryPao({{2, 9}, {5, 9}, {2, 9}, {5, 9}});
    SetTime(-95 + 110);
    pao_operator.tryPao({{1, 8.5}, {5, 8.5}});
    SetTime(600);
    pao_operator.tryRecoverPao({{2, 9}, {5, 9}});
