...
namespace AvZ
{
    ...
    void PaoOperator::base_fire_pao(int cannon_index, int drop_row, float drop_col)
    {
        ...
        Asm::paoShoot(x, y, cannon_index);
        SafeClick();
    }

    void PaoOperator::delay_fire_pao(int delay_time, int cannon_index, int row, float col)
    {
        if (delay_time != 0)
        {
            // 将操作动态插入消息队列
    ...
    void PaoOperator::autoGetPaoList()
    {
        InsertOperation([=]() {
            ...
            for (int index = 0; index < __main_object->plantCountMax(); ++index)
            {
                ...
                {
                    pao_grid = {plant_array[index].row() + 1, plant_array[index].col() + 1};
    ...
    void PaoOperator::recoverPao(int row, float col)
    {
        InsertOperation([=]() {
            ...
            update_lastest_pao_msg(__main_object->gameClock() + delay_time, next_pao);
            skip_pao(1);
        },
                        "recoverPao");
    }
